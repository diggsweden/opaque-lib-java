...
class OprfFunctionsTest {
  ...
  void testHsmOprf() throws Exception {
    ...
    OpaqueCurve opaqueCurve =
        new MockGenericOpaqueCurve(ECNamedCurveTable.getParameterSpec("P-256"), hashFunctions);
    ...
    BigInteger blind =
        new BigInteger(256, RNG).mod(parameterSpec.getCurve().getOrder().subtract(BigInteger.ONE))
            .add(BigInteger.ONE);
    ...
    ECPoint evenExpectedResult =
        multiplyWithBytePrivateKey(serverOprfKey.privateKey(), evenFirstPass);
    ...
    ECPoint evenBlindEvaluate =
        multiplyWithBytePrivateKey(serverOprfKey.privateKey(), evenBlindedK);
    ...
    assertArrayEquals(opaqueCurve.getSharedSecret(evenExpectedResult),
        opaqueCurve.getSharedSecret(evenUnblinded));
    ...
    assertArrayEquals(opaqueCurve.serializeElement(expectedResult),
        opaqueCurve.serializeElement(unblinded));
  ...
  void testP521Blind() throws Exception {
    ...
    log.info(
        TU.hex("Curve G point", curveP521.serializeElement(curveP521.getParameterSpec().getG())));
  ...
  void fixMissingYTest() throws Exception {
    ...
    byte[] xCoordinate =
        ((DefaultOprfFunction) oprf).deriveDiffieHellmanSharedSecret(serverK.getPrivate(), pwPoint);
    byte[] dhWithY = oprf.diffieHellman(new OprfPrivateKey(keyPairRecord.privateKey()),
        pwPoint.getEncoded(true));
    // byte[] xCoordinate = oprf.getCurve().getSharedSecret(oprf.deserializeElement(dhWithY));
    ECPoint evenYResult =
        oprf.deserializeElement(OpaqueUtils.concat(new byte[] {0x02}, xCoordinate));
    ECPoint oddYResult =
        oprf.deserializeElement(OpaqueUtils.concat(new byte[] {0x03}, xCoordinate));

    ECPoint controlPoint = pwPoint.add(curve.getParameterSpec().getG());
    // This control point should match the X coordinate of Z + Ks
    byte[] xControl = ((DefaultOprfFunction) oprf)
        .deriveDiffieHellmanSharedSecret(serverK.getPrivate(), controlPoint);
    ...
    byte[] completeDhOpWithY =
        oprf.diffieHellman(new OprfPrivateKey(serverK), pwPoint.getEncoded(true));
  ...
  void blindTest(OpaqueCurve opaqueCurve, HashFunctions hashFunctions) throws Exception {
    ...
    BigInteger blindInverse =
        blind.modInverse(opaqueCurve.getParameterSpec().getCurve().getOrder());
    ...
    log.info(TU.hex("Client secret = DiffieHellman(sk, (InputElement * oprfKey)",
        opaqueCurve.getSharedSecret(unblindedHsmEvaluate)));
    log.info(TU.hex("The real EC point used to derive the DH shared secret",
        unblindedHsmEvaluate.getEncoded(true)));
    ...
    log.info(TU.hex("HSM Server evaluate = DiffieHellman(ks, (blinded element * oprfKey)",
        opaqueCurve.getSharedSecret(hsmEvaluate)));
    log.info(TU.hex("ECPoint(HSM Server evaluate) =  0x02 | HSM Server evaluate",
        hsmEvaluate.getEncoded(true)));
    ECPoint evenUnblinded = hsmEvaluate.multiply(blindInverse);
    log.info(TU.hex("Client unblind = ECPoint(HSM Server evaluate) * 1/b",
        evenUnblinded.getEncoded(true)));
    log.info(TU.hex("Derived shared secret", opaqueCurve.getSharedSecret(evenUnblinded)));
    assertArrayEquals(opaqueCurve.getSharedSecret(unblindedHsmEvaluate),
        opaqueCurve.getSharedSecret(evenUnblinded));
  }
  ...
  ECPoint multiplyWithPrivateKey(KeyPair keyPair, ECPoint point, OprfFunctions oprf)
    ...
    byte[] sharedSecretPoint =
        oprf.diffieHellman(new OprfPrivateKey(keyPair), point.getEncoded(true));
