...
/**
 * Mock generic implementation of an Opaque curve. This implementation does not provide safe
 * functions for hashToScalar, hashToGroup and randomScalar. These implementations are only used for
 * testing purposes.
 */
public class MockGenericOpaqueCurve implements OpaqueCurve {
  ...
  public MockGenericOpaqueCurve(ECNamedCurveParameterSpec parameterSpec,
      HashFunctions hashFunctions) {
    this.parameterSpec = parameterSpec;
    this.hashFunctions = hashFunctions;
    this.publicKeySerializationSize = serializeElement(parameterSpec.getG()).length;
    this.privateKeySerializationSize =
        parameterSpec.getCurve().getOrder().subtract(BigInteger.ONE).toByteArray().length;
  }

  @Override
  public ECNamedCurveParameterSpec getParameterSpec() {
  ...
  @Override
  public ECPoint hashToGroup(byte[] elementData) {
