...
class OpaqueBasicTest {
  ...
  static void init() {
    ...
    sha256Identity = new HashFunctions(SHA256Digest.newInstance(),
  ...
  void basicAuthTest() throws Exception {
    ...
    performTest("Basic test with random key and seed", client, server, oprfSeed, password,
        serverKeyPair, oprfP256.getContext());
  }
  ...
  void p256TestVectorTest() throws Exception {
    ...
    String password = "650132";
    byte[] oprfSeed =
        Hex.decode("bb1cd59e16ac09bc0cb6d528541695d7eba2239b1613a3db3ade77b36280f725");
    ...
    performTest("Basic test with preset server keys", client, server, oprfSeed, password,
        serverKeyPair, oprfP256.getContext());
  }
  ...
  void sha512Test() throws Exception {
    ...
    KeyPairRecord serverKeyPair = new KeyPairRecord(
        Hex.decode("0221e034c0e202fe883dcfc96802a7624166fed4cfcab4ae30cf5f3290d01c88bf"),
        Hex.decode("34fbe7e830be1fe8d2187c97414e3826040cbe49b893b64229bab5e85a5888c7"));
    performTest("SHA-512 Opaque test", client, server, oprfSeed, password, serverKeyPair,
        oprfP256.getContext());
  }
  ...
  void performTest(
    ...
    byte[] credentialIdentifier =
        Optional.ofNullable(credentialIdentifierStr).orElse("1234").getBytes();
    byte[] password =
        Optional.ofNullable(passwordStr).orElse("S3cr3t").getBytes(StandardCharsets.UTF_8);

    log.info("Initial Values:\n{}", SLB.getInstance()
        .append(TU.hex("Passord", clientIdentity))
        .appendLine(" (" + new String(clientIdentity) + ")")
        .append(TU.hex("Client Identity", clientIdentity))
        .appendLine(" (" + new String(clientIdentity) + ")")
        .append(TU.hex("Server Identity", serverIdentity))
        .appendLine(" (" + new String(serverIdentity) + ")")
        .append(TU.hex("Credential Identifier", credentialIdentifier))
        .appendLine(" (" + new String(credentialIdentifier) + ")")
        ...
        .append(serverPrivateKey.isByteValue()
            ? TU.hex("Server Private Key", serverPrivateKey.getPrivateKeyBytes()) + "\n"
            : ""));

    RegistrationRequestResult registrationRequest = client.createRegistrationRequest(password);
    log.info("Registration request:\n{}", SLB.getInstance()
        .appendLine(
            TU.hex("Blinded message", registrationRequest.registrationRequest().blindedMessage()))
    ...
    RegistrationFinalizationResult registrationFinalizationResult =
        client.finalizeRegistrationRequest(
    ...
    log.info("Registration finalization result:\n{}", SLB.getInstance()
        ...
        .appendLine(TU.hex("Client public key",
            registrationFinalizationResult.registrationRecord().clientPublicKey()))
        .appendLine(
            TU.hex("Masking key", registrationFinalizationResult.registrationRecord().maskingKey()))
        .appendLine(TU.hex("Envelope nonce",
            registrationFinalizationResult.registrationRecord().envelope().nonce()))
        .appendLine(TU.hex("Envelope authTag",
            registrationFinalizationResult.registrationRecord().envelope().authTag())));
    ...
    log.info("Generated Ke1\n{}", SLB.getInstance()
        ...
        .appendLine(TU.hex("Credential request - blinded message",
            ke1.credentialRequest().blindedMessage()))
        .appendLine(
            TU.hex("State - Client Secret", clientState.getClientAkeState().getClientSecret()))
    ...
    log.info("Server generated Ke2\n{}", SLB.getInstance()
        .appendLine(TU.hex("Auth response - Server public key share",
        ...
        .appendLine(TU.hex("Credential Response - Masked response",
            ke2.credentialResponse().maskedResponse()))
        .appendLine(
            TU.hex("Credential Response - Masking nonce", ke2.credentialResponse().maskingNonce()))
        .appendLine(TU.hex("Credential Response - Evaluated message",
            ke2.credentialResponse().evaluatedMessage()))
        .appendLine(TU.hex("Server State - Expected client Mac",
            serverState.getAkeState().getExpectedClientMac()))
        .appendLine(
            TU.hex("Server State - Session Key", serverState.getAkeState().getSessionKey())));

    byte[] ke2Bytes = ke2.getEncoded();

    ClientKeyExchangeResult clientKeyExchangeResult =
        client.generateKe3(clientIdentity, serverIdentity, ke2Bytes,
            clientState);
